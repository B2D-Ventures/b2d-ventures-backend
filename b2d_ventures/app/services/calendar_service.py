...
class CalendarService:
  ...
  def check_availability(service, calendar_id, start_time, end_time):
      ...
      try:
          ...
          raise CalendarError(f"An error occurred while checking availability: {error}")

  @staticmethod
  def schedule_meeting(service, calendar_id, summary, description, start_time, end_time, attendees):
      """
      ...
      try:
          ...
          raise CalendarError(f"An error occurred while scheduling the meeting: {error}")
  ...
  def get_free_busy(service, time_min, time_max, calendars):
      ...
      try:
          ...
          raise CalendarError(f"An error occurred while getting free/busy information: {error}")

  @staticmethod
  def schedule_investor_startup_meeting(token, title, description, start_time, end_time, startup_email):
      """
      ...
      try:
          ...
          if not CalendarService.check_availability(service, 'primary', start_time, end_time):
              raise CalendarError("The requested time slot is not available for the investor")
